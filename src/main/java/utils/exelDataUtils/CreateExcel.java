...
public class CreateExcel {
    ...
    public CreateExcel(ArrayList<Tender> listOfTenders, String opt, String nameOfExcelFile) throws IOException {
        ...
        for (int tender = 0; tender < listOfTenders.size();tender ++) {
            ...
            String categories = "";
            for (int j = 0; j < listOfTenders.get(tender).getCategory().size(); j++) {
                categories += listOfTenders.get(tender).getCategory().get(j).toString() + " ";
            }
            cell.setCellValue(categories);
            // Category name (F)
            cell = row.createCell(8, CellType.STRING);
        ...
        DateFormat dateFormat = new SimpleDateFormat("dd-MM-yyyy HH.mm.ss");
        Date date = new Date();
        File file = new File("D:/Documents/" + nameOfExcelFile + ".xls");
        file.getParentFile().mkdirs();
